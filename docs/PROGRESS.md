...
| 18 | Automated end-to-end suite against production | 125/125 checks pass | `app/scripts/e2e.mjs` |
...
| 5 | 15 Aug | **Automated end-to-end suite written and run against production — 78 checks, 0 failures**, driving five real accounts through both lifecycles, chat privacy and authorisation (§9.1). Extended 9 Sep to 125 checks: location capture, WhatsApp, and the demo-OTP fallback. |
...
### 9.1 Automated end-to-end suite — 125 checks, 0 failures
...
extending the suite with location capture, WhatsApp, and the demo-OTP fallback: 125 passed, 0
failed.**
...
| F · Collective lifecycle (auto-assembly) | 32 | Team auto-assembled by the matching engine, coverage complete → **units split 4 + 2 = 6 respecting capacity** → **determinism: re-assembly yields the identical team** → re-assembly replaces the draft, no orphans → **provider sees nothing before confirm**, and cannot accept even by calling the API directly (409) → member removed and re-added, coverage recomputed both ways → adding beyond coverage is reported, not hidden → swap before confirm → confirm → invitations appear → one accepts, one declines → **declined member replaced on a confirmed team** → **accepted member cannot be swapped out (409)** |
| F2 · Group open call | 10 | The customer's other way to staff a group order (§11 update, 9 Sep): post with a headcount and a deadline → **appears in the same feed as individual work, not a separate one** → providers apply through the same `requests/respond` route individual work uses → **selecting more than the headcount is refused** → customer selects a subset → **picked go to accepted, the rest to declined** → applying after the deadline is refused |
...
✅ NON-participant gets 404, not 403 — status=404
  ...
  125 passed, 0 failed, 125 checks total
```
...
| Verification discipline | 125-check automated suite (0 failures) **and** a 52-check manual runbook, both against production |
...
system: 42 routes, 24 tables, a deterministic matching engine, 125 automated checks passing
