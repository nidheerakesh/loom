...
export default withHandler(async (req: VercelRequest, res: VercelResponse) => {
  ...
  const { data: interests, error } = await supabaseAdmin
    .from("interests")
    .select("state, providers(id, name, shop_name, rating, rate, rate_unit)")
  ...
  res.status(200).json(
    (interests ?? [])
      .map((i) => {
        const p = i.providers as unknown as
          | { id: string; name: string; shop_name: string | null; rating: number; rate: number | null; rate_unit: string | null }
          | null;
        if (!p) return null;
        return {
          providerId: p.id,
