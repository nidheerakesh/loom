...
export default withHandler(async (req: VercelRequest, res: VercelResponse) => {
  ...
  const { data: rows, error } = await supabaseAdmin
    .from("team_members")
    .select(
      "team_id, covered_units, state, skills(canonical_name, canonical_name_ml), teams(id, request_id, status, requests(title, status))",
    )
  ...
  type Row = {
    ...
    teams: { id: string; request_id: string; status: string; requests: { title: string; status: string } | null } | null;
  };
